...
import os
import SPECT_Dataset_Channels2
...
def channelizing_test_helper(test_dim, ref_c_tensor, verbose):
    # init result
    result = True

    values = torch.linspace(0., test_dim * test_dim - 1, steps=test_dim * test_dim, requires_grad=True)
    tensor = values.reshape(1, test_dim, test_dim)
    multiplied_tensors = []
    ...
    channelized_image = SPECT_Dataset_Channels2.channelize_tensor(test_image)
    if channelized_image.shape != (10, 1, test_dim, test_dim):
        print(f"Error in size for {test_dim}x{test_dim}: {channelized_image.shape}")
        result = False

    # check the values
    for i in range(10):
        this_tensor = channelized_image[i, :, :, :]  # .view(-1, 1, 1, 16)
        ref_tensor = (i + 1) * ref_c_tensor.view(-1, 1, test_dim, test_dim)
        condition = torch.all(torch.abs(this_tensor - ref_tensor) < 0.01)
        if not condition:
            print(f"Unexpected values for {test_dim}x{test_dim} tensor in channel {i}: {this_tensor}; {ref_tensor}.")
            result = False

    # de-channelize and see if values match
    dechannelized_image = SPECT_Dataset_Channels2.dechannelize_tensor(channelized_image)
...
def channelizing_test(verbose):
    # test 2x2
    test_dim = 2
    ref_c_tensor = torch.tensor([1.5, -1.5, -0.5, 0.5])
    if not channelizing_test_helper(test_dim, ref_c_tensor, verbose):
        print(f"Error during channelizing test {test_dim}x{test_dim}.")
    else:
        print(f"Test {test_dim}x{test_dim} passed.")

    # test 4x4
    test_dim = 4
    ref_c_tensor = torch.tensor(
        [7.5, -5.0, -2.5, -1.5, 1.5, -3.0, -2.5, -1.5, 1.5, 3.0, -2.5, -1.5, 1.5, -2.5, -1.5, 1.5]).view(1, 1, 4, 4)
    if not channelizing_test_helper(test_dim, ref_c_tensor, verbose):
        print(f"Error during channelizing test {test_dim}x{test_dim}.")
    else:
        print(f"Test {test_dim}x{test_dim} passed.")
    ...
    values = torch.linspace(0., 4 * 4 - 1, steps=4 * 4, requires_grad=True)
    tensor = values.reshape(1, 4, 4)
    multiplied_tensors = []
    ...
    channelized_image = SPECT_Dataset_Channels2.channelize_tensor(test_image)
    if channelized_image.shape != (10, 1, 4, 4):
        print(f"Error in size for 4x4: {channelized_image.shape}")

    # check the values
    for i in range(10):
        this_tensor = channelized_image[i, :, :, :]  # .view(-1, 1, 1, 16)
        ref_tensor = torch.tensor(
            [7.5, -5.0, -2.5, -1.5, 1.5, -3.0, -2.5, -1.5, 1.5, 3.0, -2.5, -1.5, 1.5, -2.5, -1.5, 1.5])
        ref_tensor = (i + 1) * ref_tensor.view(-1, 1, 4, 4)
        condition = torch.all(torch.abs(this_tensor - ref_tensor) < 0.01)
        if not condition:
            print(f"Unexpected values for 4x4 tensor in channel {i}: {this_tensor}; {ref_tensor}.")

    # dec-channelize and see if values match
    dechannelized_image = SPECT_Dataset_Channels2.dechannelize_tensor(channelized_image)
    if not torch.all(torch.abs(dechannelized_image - test_image) < 0.01):
        print("De-channelized values for 4x4 did not match.")

    values = torch.linspace(0., 8 * 8 - 1, steps=8 * 8, requires_grad=True)
    test_image = values.reshape(1, 1, 8, 8)
    channelized_image = SPECT_Dataset_Channels2.channelize_tensor(test_image)
    ...
    pass5 = True
    dechannelized_image = SPECT_Dataset_Channels2.dechannelize(channelized_image)
